...
const EditorProducto = ({ cerrarDialogo, producto, actualizarProductos, setActualizarProductos}) => {
    ...
    const persistirProducto = () =>{
        ...
        //const url = "http://localhost:8080/api/productos" + (productoEdicion.id ? `/${productoEdicion.id}` : "");
        const url = API_ENDPOINTS.productos + (productoEdicion.id ? `/${productoEdicion.id}` : "");
        fetch(url,
            {
                method: metodo,
